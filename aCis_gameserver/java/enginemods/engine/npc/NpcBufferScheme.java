...
public class NpcBufferScheme extends AbstractMods
{
	...
	private static boolean FREE_BUFFS = ConfigData.NPC_BUFFER_FREE_BUFFS;
	
	// precio de usar cada buff, sea del tipo q sea.
	private static int BUFF_PRICE = ConfigData.NPC_BUFFER_BUFF_PRICE;
	
	// precio de los buffs pre-establecidos
	private static int BUFF_SET_PRICE = ConfigData.NPC_BUFFER_BUFF_SET_PRICE;
	// item a cobrar
	private static int CONSUMABLE_ID = ConfigData.NPC_BUFFER_CONSUMABLE_ID;
	...
	private static int BUFF_REMOVE_PRICE = ConfigData.NPC_BUFFER_BUFF_REMOVE_PRICE;
	private static int SCHEME_BUFF_PRICE = ConfigData.NPC_BUFFER_SCHEME_BUFF_PRICE;
	private static int SCHEMES_PER_PLAYER = ConfigData.NPC_BUFFER_SCHEMES_PER_PLAYER;
	
	// maximo de buffs q se pueden agregar de cada tipo por scheme
	private static int MAX_SCHEME_BUFFS = ConfigData.NPC_BUFFER_MAX_SCHEME_BUFFS;
	private static int MAX_SCHEME_DANCE = ConfigData.NPC_BUFFER_MAX_SCHEME_DANCE;
	...
	{	
	    ...
	    if (SkillData.CUSTOM_ICONS.containsKey(id)) // Acceso a CUSTOM_ICONS a través de SkillData
	    {
	        ...
	        return "<button action=\"bypass -h Engine NpcBufferScheme description " + id + " " + level + " x\" width=32 height=32 back=\"" + iconName + "\" fore=\"" + iconName + "\">"; // Devuelve el botón con el icono personalizado
	    }
	...
	private void rebuildMainHtml(Player player)
	{
		...
		hb.append("<td height=32 align=center><button value=Dance action=\"bypass -h Engine NpcBufferScheme redirect_view_dance\" width=75 height=21back=", L2UI_CH3.Btn1_normalOn, " fore=", L2UI_CH3.Btn1_normal, "></td>");
		hb.append("</tr>");
		hb.append("</table>");
		hb.append("<br>");
